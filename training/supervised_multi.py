...
def _embed_shard(encoder: nn.Module, graphs, batch: int, device: str) -> np.ndarray:
    ...
    for i in range(0, len(graphs), batch):
        ...
        chunks.append(H)
    return np.concatenate(chunks, axis=0) if chunks else np.zeros((0, getattr(encoder, "hidden_dim", 1)), dtype=np.float32)

def _worker_embed(rank: int, world: int, encoder_state: dict, graphs, batch: int, device_ids: List[int], out_path: str):
    ...
    # shard indices
    n = len(graphs); start = (n * rank) // world; end = (n * (rank + 1)) // world
    X = _embed_shard(enc, graphs[start:end], batch, device)
    np.save(f"{out_path}.rank{rank}.npy", X)

def compute_embeddings_multi_gpu(
    encoder: nn.Module, dataset: GraphDataset, batch: int, device_ids: Optional[List[int]] = None
) -> np.ndarray:
    """Shard dataset across GPUs, compute embeddings in parallel, then concatenate."""
    if device_ids is None or len(device_ids) <= 1:
        return _embed_shard(encoder, dataset.graphs, batch, device="cuda:0" if torch.cuda.is_available() else "cpu")
    world = len(device_ids)
    tmp = Path("outputs/emb_tmp"); tmp.mkdir(parents=True, exist_ok=True)
    out_prefix = str(tmp / "emb")
    # package minimal cfg so workers can rebuild the encoder
    cfg = dict(gnn_type=getattr(encoder, "gnn_type", "mpnn"),
    ...
    state = copy.deepcopy(encoder.state_dict()); state["_cfg"] = cfg
    ctx = mp.get_context("spawn")
    procs = []
    for r in range(world):
        p = ctx.Process(target=_worker_embed, args=(r, world, state, dataset.graphs, batch, device_ids, out_prefix))
...
class LinearHead(nn.Module):
    ...
    def forward(self, x): return self.out(x).squeeze(-1)
...
def train_linear_head_earlystop(
    ...
    X_tr = compute_embeddings_multi_gpu(encoder, train_ds, batch=batch_size, device_ids=multi_gpu_ids)
    X_va = compute_embeddings_multi_gpu(encoder, val_ds,   batch=batch_size, device_ids=multi_gpu_ids)
