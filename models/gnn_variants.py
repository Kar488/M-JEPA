...
class GraphSAGELayer(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, agg: str = "mean", dropout: float = 0.0):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        ...
        if self.agg == "mean":
            deg = torch.zeros(x.size(0), device=x.device).index_add_(0, i, torch.ones_like(i, dtype=torch.float))
            agg.index_add_(0, i, x[j])
            deg = deg.clamp(min=1.0).unsqueeze(-1)
            agg = agg / deg
        elif self.agg == "max":
            agg.fill_(-1e9)
            agg = torch.max(agg.index_copy(0, i, x[j]), dim=0).values  # rough max; simple fallback
        else:
...
class GraphSAGE(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, num_layers: int = 3, agg: str = "mean", dropout: float = 0.1):
        super().__init__()
        self.proj = nn.Linear(input_dim, hidden_dim)
        self.layers = nn.ModuleList([GraphSAGELayer(hidden_dim, hidden_dim, agg=agg, dropout=dropout) for _ in range(num_layers)])
        self.out_norm = nn.LayerNorm(hidden_dim)

    def encode(self, g: GraphData, device: torch.device) -> torch.Tensor:
    ...
    def forward(self, batch: Union[GraphData, List[GraphData]]) -> torch.Tensor:
        ...
        if isinstance(batch, list):
            embs = [self.encode(g, device) for g in batch]
            return torch.stack(embs, dim=0)
        return self.encode(batch, device).unsqueeze(0)
...
class MultiHeadGATLayer(nn.Module):
    def __init__(self, dim: int, heads: int = 4, dropout: float = 0.0, concat: bool = True):
        super().__init__()
        self.heads = heads
        self.concat = concat
        ...
        nn.init.xavier_uniform_(self.attn_src); nn.init.xavier_uniform_(self.attn_dst)
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        ...
        # scores
        alpha = (xh[i] * self.attn_dst).sum(-1) + (xh[j] * self.attn_src).sum(-1)  # [E, H]
        ...
        try:
            ...
            denom = segment_softmax(alpha_exp, i) * 0 + alpha_exp  # not ideal; fallback below
        except Exception:
...
class GATMultiHead(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, num_layers: int = 3, heads: int = 4, dropout: float = 0.1):
        super().__init__()
        self.proj = nn.Linear(input_dim, hidden_dim)
        self.layers = nn.ModuleList([MultiHeadGATLayer(hidden_dim, heads=heads, dropout=dropout, concat=True) for _ in range(num_layers)])
        self.out_norm = nn.LayerNorm(hidden_dim)
