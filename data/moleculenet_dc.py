...
def download_moleculenet_to_parquet(
    ...
    """
    if not _ensure_dc():
        raise RuntimeError("DeepChem not installed. Please `pip install deepchem` (and rdkit).")
    ...
    if name_low not in loaders:
        raise ValueError(f"Unsupported dataset {name}. Supported: {list(loaders.keys())}")
