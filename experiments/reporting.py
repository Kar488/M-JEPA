...
def summarize_with_ci(
    df: pd.DataFrame, metrics: List[str], seeds_col: str = "seeds"
) -> pd.DataFrame:
    out = df.copy()
...
def plot_topn_bar(
    ...
    labels = (
        (
            df2["method"]
            + " | hid="
            + df2["hidden_dim"].astype(str)
            + " L="
            + df2["num_layers"].astype(str)
            + " m="
            + df2["mask_ratio"].astype(str)
        ).tolist()
        if "method" in df2.columns
        else df2.index.astype(str).tolist()
    )
...
def save_ranked_csv(
    df: pd.DataFrame, metric: str, out_csv: str, top_n: int = 50
) -> None:
    df.sort_values(metric, ascending=False).head(top_n).to_csv(out_csv, index=False)


def build_full_report(
    df: pd.DataFrame, metric: str, out_dir: str, top_n: int = 20
) -> None:
    _ensure_dir(out_dir)
    save_ranked_csv(
        df,
        metric=metric,
        out_csv=os.path.join(out_dir, f"ranked_top{top_n}.csv"),
        top_n=top_n,
    )
    plot_topn_bar(
        df,
        metric=metric,
        top_n=top_n,
        out_path=os.path.join(out_dir, f"top{top_n}_bar.png"),
