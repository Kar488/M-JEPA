...
def concat_dir_to_csv(
    dirpath: str, smiles_col: str, label_col: Optional[str], out_csv: str
) -> str:
...
def baseline_pretrain_and_embed(
    ...
    if not os.path.exists(emb_out) or force_embed:
        cli.embed(
            method, ckpt_path=ckpt_path, smiles_file=smiles_eval_file, emb_out=emb_out
        )
    return ckpt_path, emb_out
