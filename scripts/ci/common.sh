...
ci_cleanup_resolve_reason() {
  local cmdline="$1"
  local env_blob="$2"
  local allow_stage_fallback="$3"
  local stage_tokens_name="$4"
  shift 4
  local -n __stage_tokens="$stage_tokens_name"
  local -a tokens=("$@")
  local reason=""
  local stage_reason=""
  local has_exp_marker=0
  if ci_cleanup_has_exp_id_marker "$cmdline" "$env_blob"; then
    has_exp_marker=1
  fi
  reason="$(ci_cleanup_match_reason "$cmdline" "$env_blob" "${tokens[@]}" || true)"
  if [[ ${#__stage_tokens[@]} -gt 0 ]]; then
    if [[ "$allow_stage_fallback" == "1" ]] || (( ! has_exp_marker )); then
      stage_reason="$(ci_cleanup_match_stage_reason "$cmdline" "${__stage_tokens[@]}" || true)"
    fi
  fi
  if [[ -n "$stage_reason" ]]; then
    if (( ! has_exp_marker )); then
      reason="$stage_reason"
    elif [[ -z "$reason" ]]; then
      reason="$stage_reason"
    fi
...
ci_cleanup_lock_info() {
  ...
  __out_exp_id="$(grep -E '^EXP_ID=' "$lock_path" 2>/dev/null | head -n1 | cut -d= -f2- || true)"
  return 0
}
...
ci_cleanup_stage_processes() {
  ...
  if declare -F ci_cleanup_lock_info >/dev/null 2>&1; then
    ci_cleanup_lock_info "$stage" lock_pid lock_pgid lock_exp_id
  fi
  ...
  if [[ -n "$lock_pid" && -n "$lock_pgid" ]]; then
    if kill -0 "$lock_pid" 2>/dev/null; then
      ...
      for lock_proc in /proc/[0-9]*; do
        ...
        lock_reason="$(ci_cleanup_resolve_reason \
          "$lock_cmdline" \
          "$lock_env" \
          "$allow_stage_fallback" \
          stage_tokens \
          "${match_tokens[@]}" \
          "${env_tokens[@]}")"
  ...
  for proc in /proc/[0-9]*; do
    ...
    reason="$(ci_cleanup_resolve_reason \
      "$cmdline" \
      "$env_blob" \
      "$allow_stage_fallback" \
      stage_tokens \
