...
# Emit a one-line, space-separated CLI args file for pretrain/finetune
BEST_ARGS="${GRID_DIR:-$APP_DIR/grid}/phase2_best_args.txt"
python - <<'PY'
import json, os, shlex, sys
best = json.load(open(os.environ["BEST_JSON"]))["best"]
cfg  = best["config"]

# allow-list of fixed knobs to propagate downstream
keep = [
  "training_method",
  "gnn_type", "hidden_dim", "num_layers", "contiguity", "add_3d",
  "learning_rate", "pretrain_batch_size", "finetune_batch_size",
  "pretrain_epochs", "finetune_epochs",
  "mask_ratio", "ema_decay",            # JEPA
  "temperature",                        # Contrastive
]
# map underscored keys → hyphen flags expected by pretrain CLI helpers
MAP = {
  "pretrain_batch_size": "pretrain-batch-size",
  "finetune_batch_size": "finetune-batch-size",
...
for k in keep:
  if k in cfg and cfg[k] is not None:
    key = MAP.get(k, k).replace("_","-")
    val = cfg[k]
    # booleans/ints/floats → string; booleans as 0/1
    if isinstance(val, bool): val = "1" if val else "0"
    else: val = str(val)
    parts += [f"--{key}", val]

# Always include task-type if present in Phase-2 config
if "task_type" in cfg:
  parts += ["--task-type", str(cfg["task_type"])]

out = " ".join(shlex.quote(p) for p in parts)
open(os.environ["BEST_ARGS"], "w").write(out + "\n")
print("[phase2] wrote best args:", out)
PY
