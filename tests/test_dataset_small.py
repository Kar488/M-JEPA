...
def test_dataset_from_file_smoke(tiny_parquet, add_3d):
    ...
    if ext == ".parquet":
        ds = GraphDataset.from_parquet(path, smiles_col="smiles", cache_dir=None, add_3d_features=add_3d, n_rows=10)
    else:
        ds = GraphDataset.from_csv(path, smiles_col="smiles", cache_dir=None, add_3d_features=add_3d, n_rows=10)
