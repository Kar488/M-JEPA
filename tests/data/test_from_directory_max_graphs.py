...
def test_from_directory_respects_max_graphs(tmp_path):
    ...
    with patch.object(
        GraphDataset, "from_parquet", wraps=GraphDataset.from_parquet
    ) as mock_fp:
        ds = GraphDataset.from_directory(str(tmp_path), max_graphs=5, n_rows_per_file=2)

    assert len(ds) <= 5
    touched = [
        os.path.basename(call.kwargs["filepath"]) for call in mock_fp.call_args_list
    ]
