...
def test_pin_visible_cuda_device_reuses_snapshot_with_canonical_variants(monkeypatch):
    monkeypatch.delenv("DISABLE_DDP", raising=False)
    monkeypatch.setenv("WORLD_SIZE", "2")
    monkeypatch.setenv("LOCAL_WORLD_SIZE", "2")
    monkeypatch.setenv("LOCAL_RANK", "1")
    monkeypatch.setenv("CUDA_VISIBLE_DEVICES", "cuda:4, cuda:5")
    ...
    first = ddp._pin_visible_cuda_device_to_local_rank()
    assert first == "cuda:5"
    assert os.environ["CUDA_VISIBLE_DEVICES"] == "cuda:5"
    assert fake_cuda.set_calls == [0]

    # Simulate another launch where the CUDA mask was normalised externally.
    os.environ["CUDA_VISIBLE_DEVICES"] = "5"

    second = ddp._pin_visible_cuda_device_to_local_rank()
    assert second == "cuda:5"
    assert os.environ["CUDA_VISIBLE_DEVICES"] == "cuda:5"
    assert fake_cuda.set_calls == [0, 0]

    ddp.cleanup()
    assert os.environ["CUDA_VISIBLE_DEVICES"] == "cuda:4, cuda:5"
    assert ddp._CUDA_VISIBLE_DEVICE_STACK == []
