...
def test_plot_training_curves_returns_figure(wb):
    ...
    fig = plot_training_curves(curves, title="Test Training curve", wb=wb)
...
def test_plot_hyperparameter_results_empty(wb):
    df = pd.DataFrame()
    assert plot_hyperparameter_results(df, metric="roc_auc", title="Test Hyperparameter Search Results", wb=wb) is None


def test_plot_hyperparameter_results_rmse(wb):
    df = pd.DataFrame({"hidden_dim": [32, 64], "rmse": [0.5, 0.3]})
    fig = plot_hyperparameter_results(df, metric="rmse", title="Hyperparameter Search Results", top_n=0, wb=wb)
    assert isinstance(fig, Figure)
