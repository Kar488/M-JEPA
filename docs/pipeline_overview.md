...
1. **Pretrain.** ``pretrain-agent`` writes encoder checkpoints and seeds the
   lineage.
2. **Phase 1 sweep.** ``phase1-agent`` runs the coarse sweep, writing into
   ``$EXPERIMENTS_ROOT/$GRID_EXP_ID/grid/phase1``.
3. **Phase 2 sweep.** ``phase2-agent`` rechecks and exports winning configs,
   writing ``grid/phase2_*`` artifacts.
4. **Tox21 grading (benchmark stage).** ``tox21-agent`` evaluates the encoder on
   Tox21 tasks and writes ``bench/encoder_frozen.ok`` on success.
5. **Finetune / Benchmark / Report.** ``finetune-agent`` and ``report-agent``
...
For policy details and override semantics, read
